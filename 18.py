...
def group():
    ...
    items = [level.split(" ") for level in levels]
    items = [[int(digit) for digit in level] for level in items]
    return items
def highest(items):
    max = 0
...
def maxUP(tree):
    for depth in range(len(tree) - 2, -1, -1):
        for position in range(len(tree[depth])):
            right = tree[depth + 1][position+1]
            left = tree[depth + 1][position]
            tree[depth][position] += max(right,left)
    return tree[0][0]
