...
def process():
    ...
    grid = [[int(number) for number in row] for row in grid]
    return grid
...
def group(n):
    ...
    for row in range(rows - n + 1):
        for column in range(columns):
            ...
            spaceRight = column + (n - 1) < columns
            spaceLeft =  column - (n - 1) >= 0
            for i in range(n):
                down.append(grid[row + i][column]);
                if spaceRight:
                        right.append(grid[row][column + i]);
                        rDia.append(grid[row+i][column + i]);
                if spaceLeft:
                    lDia.append(grid[row+i][column - i]);
...
if __name__ == "__main__":
    process()
    groups = group(4)
    products = [product(group) for group in groups]
    products.sort();
    print(products[-1])
